...
def main():
    ...
    conf_blob = bucket.blob(f'{args.uid}/{args.config}')
    yaml_file = conf_blob.download_as_bytes()
    config = Configuration().fromYamlString(yaml_file.decode("utf8"))

    #Read observations from GCS
    preprocessed_observations = pd.read_csv(
        f'gs://{args.bucket}/{args.uid}/{args.preprocessed_observations}',
        index_col=False,
        dtype={
            "obs_id" : str
        }
    ...
    #Read test orbits from GCS
    test_orbits = Orbits.from_csv(f'gs://{args.bucket}/{args.uid}/{args.test_orbits}')
